...
class EnvEditor
{
    ...
    {
        $backupPath      = Str::finish(config('enveditor.backupPath'), '/');
        $env             = config('enveditor.pathToEnv');
        $filePermissions = config('enveditor.filePermissions');
    ...
    public function getValue($key)
    {
        if ($this->keyExists($key)) {
            ...
            throw new EnvException('Requested value not available');
        }
    }
    ...
    public function setAutoBackup($onOff)
    {
        if (is_bool($onOff)) {
            ...
            throw new EnvException('Auto backup wrong value');
        }
    }
    ...
    protected function getFile($timestamp)
    {
        ...
        if (file_exists($file)) {
            ...
            throw new EnvException('Requested backup not found');
        }
    ...
    public function getBackupVersions()
    {
        ...
        throw new EnvException('No backups available');
    }
    ...
    public function deleteBackup($timestamp)
    {
        ...
        if (file_exists($file)) {
            ...
            throw new EnvException('backup not deletable');
        }
    }
    ...
    public function changeEnv($data = array())
    {
        ...
        throw new EnvException('Array needed');
    }
    ...
    public function deleteData($data = array())
    {
        foreach ($data as $key => $value) {
            if (!is_numeric($key)) {
                throw new EnvException("dotenv-editor::class.numeric_array_needed", 0);
            }
        }
